...
func TestCleanupVolumeSnapshot(t *testing.T) {
		...
		},
		{
			...
			objs: []runtime.Object{
				&snapshotv1api.VolumeSnapshot{
					...
					Status: &snapshotv1api.VolumeSnapshotStatus{
						BoundVolumeSnapshotContentName: ptr.To("retain-vsc"),
					},
				},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.volSnap.Status != nil && tc.volSnap.Status.BoundVolumeSnapshotContentName != nil {
				actualVSC := new(snapshotv1api.VolumeSnapshotContent)
				err := fakeClient.Get(
					t.Context(),
					crclient.ObjectKey{Name: *tc.volSnap.Status.BoundVolumeSnapshotContentName},
					actualVSC,
				)
				require.NoError(t, err)
