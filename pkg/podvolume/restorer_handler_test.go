...
//
// What happens on a second terminal transition depends only on the channel
// occupancy at send time (restorer.go:119):
//   - buffer full, no consumer   -> the duplicate send is dropped with an
//     error log and the handler returns promptly (a blocking send here would
//     park the informer's event dispatch goroutine forever — the send-side
//     residual of the #10250 deadlock family);
//   - buffer drained (loop done) -> the send succeeds and the re-transitioned
//     result is orphaned in the buffer, never seen by tracker.TrackPodVolume;
//   - buffer slack (numRestores < len(volumesToRestore)) -> the spare capacity
//     absorbs the extra send.
func TestRestorerHandlerRetransitionOverflow(t *testing.T) {
	const numVolumes = 2

	t.Run("full buffer: terminal re-transition send is dropped without blocking", func(t *testing.T) {
		informer, r, hook := newHandlerHarness(t)
		ch := registerSizedResultsChannel(t, r, numVolumes)

		// numRestores == numVolumes normal terminal transitions. With no
		// consumer running, the buffer (capacity == numVolumes) fills
		// deterministically.
		...
		case <-retransitionDone:
			// Handler returned promptly: the re-transition send must never
			// block on a full results buffer — one manual CR edit with a
			// full buffer must not stall the informer's event dispatch
			// goroutine permanently.
		case <-time.After(2 * time.Second):
			t.Fatal("re-transition send blocked on the full buffer: the handler goroutine is parked on resChan <- pvr")
		}

		// The re-transitioned result is dropped: the buffer still holds only
		// the original numVolumes results and no extra value was appended.
		require.Len(t, ch, numVolumes)

		// The drop is observable, pinning that the duplicate terminal result
		// was discarded deliberately rather than sent into the buffer.
		require.True(t, hook.containsMessage("results channel is full"))
	})

	t.Run("drained buffer: re-transition send succeeds and the result is orphaned", func(t *testing.T) {
		informer, r, _ := newHandlerHarness(t)
		ch := registerSizedResultsChannel(t, r, numVolumes)
		...
		select {
		case <-retransitionDone:
			// Handler returned promptly: the send fit the drained buffer.
		case <-time.After(2 * time.Second):
			t.Fatal("re-transition send blocked even though the buffer was drained")
		}

		// The re-transitioned result is delivered into the buffer even though
		// the receive loop has already finished: nobody will ever consume it
		// and tracker.TrackPodVolume never sees it. A per-PVR terminal dedup
		// would keep this channel empty.
		select {
		case got := <-ch:
			require.Equal(t, "pvr-0", got.Name)
			require.Equal(t, velerov1api.PodVolumeRestorePhaseCanceled, got.Status.Phase)
		default:
			t.Fatal("expected the re-transitioned PVR to be buffered as an orphaned result")
		}
	})

	t.Run("buffer slack absorbs the re-transition send", func(t *testing.T) {
		informer, r, _ := newHandlerHarness(t)
		...
		select {
		case <-retransitionDone:
			// Handler returned promptly: the spare capacity absorbed the
			// extra send — this is why the defect only manifests on the
			// maximal path (buffer slack zero).
		case <-time.After(2 * time.Second):
			t.Fatal("re-transition send blocked despite free buffer slack")
		}

		// Both sends are buffered: the original result and the re-transition.
		count := 0
	drain:
		for {
			select {
