...
// TestBatchDeleteSnapshotsS3ErrorPaths covers the error branches inside the
// per-repositoryType loop: an EnsureRepo failure for one type must append an
// error and continue with the other type, and a BatchForget failure must
// surface its errors while the other type still completes.
func TestBatchDeleteSnapshotsS3ErrorPaths(t *testing.T) {
	backup := builder.ForBackup("velero", "s3-del-backup").
		StorageLocation("s3-bsl").
		Result()
	...
	mixedSnapshots := map[string][]repotypes.SnapshotIdentifier{
		"ns1": {
			{SnapshotID: "s3-snap-kopia-1", VolumeNamespace: "ns1", BackupStorageLocation: "s3-bsl", RepositoryType: "kopia"},
			{SnapshotID: "s3-snap-rst-1", VolumeNamespace: "ns1", BackupStorageLocation: "s3-bsl", RepositoryType: "rst"},
		},
	...
	t.Run("EnsureRepo failure for one type still processes the other", func(t *testing.T) {
		// Only the kopia repo exists; the rst repo is missing, so EnsureRepo
		// must fail for rst while kopia still gets its BatchForget call.
		fakeClient := velerotest.NewFakeControllerRuntimeClient(t, s3ReadyRepo("kopia"))
		...
		errs := batchDeleteSnapshots(t.Context(), repository.NewEnsurer(fakeClient, velerotest.NewLogger(), 2*time.Second), repoMgr, mixedSnapshots, backup, velerotest.NewLogger())

		require.NotEmpty(t, errs, "the missing rst repository must surface an error")
		assert.Contains(t, errs[0].Error(), "error to ensure repo", "the EnsureRepo failure is wrapped")
		assert.Equal(t, 1, calls, "the kopia repository must still be batch-forgotten")
	})

	t.Run("BatchForget error for one type still processes the other", func(t *testing.T) {
		fakeClient := velerotest.NewFakeControllerRuntimeClient(t, s3ReadyRepo("kopia"), s3ReadyRepo("rst"))

		var calls int
		repoMgr := repomocks.NewManager(t)
		repoMgr.On("BatchForget", mock.Anything, mock.MatchedBy(func(repo *velerov1api.BackupRepository) bool {
			return repo.Name == "ns1-s3-bsl-rst"
		}), mock.Anything).
			Return([]error{fmt.Errorf("s3: rst forget failed")}).
			Run(func(args mock.Arguments) { calls++ })
		repoMgr.On("BatchForget", mock.Anything, mock.MatchedBy(func(repo *velerov1api.BackupRepository) bool {
			return repo.Name == "ns1-s3-bsl-kopia"
		}), mock.Anything).
			Return(nil).
			Run(func(args mock.Arguments) { calls++ })
		...
		require.Len(t, errs, 1, "only the rst BatchForget error must be reported")
		assert.Contains(t, errs[0].Error(), "rst forget failed")
		assert.Equal(t, 2, calls, "both repositories must still be attempted")
	})
}
