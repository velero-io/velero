...
- `--graceful-shutdown-timeout` overrides the derived value explicitly. It must be a positive duration (for example `45m`); an explicit `0` or negative value is rejected at startup. Because this value is not validated against the pod's `terminationGracePeriodSeconds`, an override that is equal to or greater than it will be cut short by Kubernetes force-killing the pod before the manager finishes waiting; size it below the pod's `terminationGracePeriodSeconds` (see the `preStop` note below).
- `--graceful-shutdown-safety-buffer` controls the buffer subtracted from `terminationGracePeriodSeconds` when deriving the timeout automatically, applied uniformly (including when `terminationGracePeriodSeconds` matches Kubernetes' own 30 second default), so the derived timeout always stays strictly below the pod's grace period. It defaults to 2 seconds. If the pod defines a `preStop` lifecycle hook, size this buffer to also cover the hook's maximum duration: the pod's `terminationGracePeriodSeconds` countdown starts before the hook runs, and SIGTERM (which triggers the manager's shutdown) is only sent once the hook completes, so the hook and the manager's shutdown draw from the same budget. Note that this buffer is a `velero server` safety margin, not a Kubernetes-documented mechanism — Kubernetes itself only recommends sizing `terminationGracePeriodSeconds` generously enough to cover the `preStop` hook plus the application's own shutdown time.
- If the pod's `terminationGracePeriodSeconds` can't be determined at all (for example, the `POD_NAME` environment variable isn't set), `velero server` logs a warning and falls back to a 30 second timeout, matching its historical default. If it *can* be determined but is too short for the safety buffer to fit, it falls back to the pod's own `terminationGracePeriodSeconds` with no safety margin, rather than a flat 30 seconds — this still never exceeds what Kubernetes will actually allow, while preserving as much of a grace period the operator deliberately raised (for example, to give backups/restores more time to finish on node eviction) as possible.
